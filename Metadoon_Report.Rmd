...
run_deseq2 <- function(ps) {
  ...
  get_tax_label <- function(tax_mat) {
    apply(tax_mat, 1, function(x) {
      paste(na.omit(x[c("Genus", "Family", "Order")]), collapse=";")
    })
  }
  ...
  for (meta_var in meta_columns) {
    ...
    tryCatch({
      ...
      if (nrow(res_sig) > 0) {
        ...
        print(p)
        cat(paste0("\nFound ", nrow(res_sig_df), " significant taxa (padj < 0.05).\n"))
      } else {
...
run_ancombc <- function(ps) {
  ...
  for (meta_var in meta_columns) {
    ...
    tryCatch({
      # --- CORREÇÃO CRÍTICA PARA ERRO DE 'get' ---
      # Removemos NAs sem usar get(). Acessamos a coluna diretamente no sample_data
      keep_samples <- !is.na(sample_data(ps)[[meta_var]])
      ps_sub <- prune_samples(keep_samples, ps)
      
      # Remove taxa vazios após o subset
      ps_sub <- prune_taxa(taxa_sums(ps_sub) > 0, ps_sub)
      
      # Run ANCOM-BC
      out <- ancombc(phyloseq = ps_sub, 
                     formula = meta_var, # Pass string name directly
                     p_adj_method = "holm", 
                     lib_cut = 0, 
                     group = meta_var, 
                     struc_zero = TRUE, 
