...
run_ancombc <- function(ps) {
  ...
  meta_data <- as(sample_data(ps), "data.frame")
  # Filter metadata columns (remove SampleID)
  meta_columns <- setdiff(colnames(meta_data), "SampleID")
  
  for (meta_var in meta_columns) {
    # Check for sufficient groups (at least 2 groups with data)
    # We remove NAs to check if there are enough valid levels
    if (length(unique(na.omit(meta_data[[meta_var]]))) < 2) next
    
    # Print Header for the variable using Markdown syntax
    ...
    tryCatch({
      ...
      # Prune taxa that are not present in any of the remaining samples
      ps_sub <- prune_taxa(taxa_sums(ps_sub) > 0, ps_sub)
      
      # 2. Run ANCOM-BC
      # Using standard parameters suitable for microbiome data
      out <- ancombc(phyloseq = ps_sub, formula = meta_var, 
                     p_adj_method = "holm", 
                     lib_cut = 0, # Set to 0 to avoid dropping small libraries if rarefaction wasn't used
                     group = meta_var, 
                     struc_zero = TRUE, 
                     neg_lb = TRUE,
                     tol = 1e-5, 
      ...
      # 3. Process Results
      # 'diff_abn' contains TRUE/FALSE for significance
      diff_df <- res$diff_abn
      
      # Find taxa that are significant in at least one contrast
      sig_taxa_indices <- apply(diff_df, 1, any)
      
      if (sum(sig_taxa_indices) > 0) {
        
        # Extract Beta (Effect Size) and Q-values
        beta_df <- res$beta[sig_taxa_indices, , drop = FALSE]
        qval_df <- res$q_val[sig_taxa_indices, , drop = FALSE]
        
        # Prepare Plot Data
        # We visualize the primary contrast (first column of coefficients)
        df_plot <- data.frame(
        ...
        cat(paste0("\n\n> **Result:** ANCOM-BC identified ", nrow(df_plot), " significant taxa (adjusted p < 0.05).\n\n"))
        
      } else {
        cat(paste0("\n\n> No significant taxa found by ANCOM-BC for **", meta_var, "** (padj < 0.05).\n\n"))
      }
      
    }, error = function(e) {
      # This ensures the error is visible in the report
