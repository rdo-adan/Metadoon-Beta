...
```
```{r load-data}
# Load data as in existing script
...
```{r abundance-plots, results='hide', echo=FALSE}
...
```{r core-heatmaps, results='hide', echo=FALSE}
...
plot_alpha_diversity <- function(ps) {
  ...
  if (length(meta_file) == 0) stop("No metadata file found.")
...
## NMDS:
```{r beta-nmds, echo=FALSE}
plot_beta_diversity <- function(ps, dist_method = dist_method) {
...
plot_beta_diversity(ps, dist_method)

```
## PCoA:
```{r beta-pcoa, echo=FALSE}
